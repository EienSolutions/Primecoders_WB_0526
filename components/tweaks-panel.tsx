...
export function TweaksPanel({ title = 'Tweaks', children }: TweaksPanelProps) {
  // Always-open in dev — no host iframe protocol in Next.js context.
    ...
    return (
      <button
        ...
        style={{
          ...
          appearance: 'none', height: 30, padding: '0 12px', border: 0, borderRadius: 8,
          background: 'rgba(0,0,0,.78)', color: '#fff', fontSize: 12, cursor: 'pointer',
...
}: { label: string; value?: ReactNode; children?: ReactNode; inline?: boolean }) {
  return (
...
export function TweakRadio<T extends string | number | boolean>({
  ...
  const fitsAsSegments = maxLen <= ({ 2: 16, 3: 10 } as Record<number, number>)[options.length];

  if (!fitsAsSegments) {
    const resolve = (s: string): T => {
      const m = options.find((o) =>
        String(typeof o === 'object' && o !== null && 'value' in o ? o.value : o) === s,
      );
    ...
    return (
      <TweakSelect
        ...
        options={options as unknown as TweakOption<string>[]}
  ...
  return (
    <TweakRow label={label}>
      ...
      >
        ...
        {opts.map((o) => (
          <button
            key={String(o.value)} type="button" role="radio"
    ...
    <TweakRow label={label}>
      <div className="twk-chips" role="radiogroup">
        {options.map((o) => {
          ...
          const chipStyle: CSSProperties = { background: o };
          return (
            <button
              key={o} type="button" className="twk-chip" role="radio"
